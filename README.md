...
| `openmrs-backend.db.database`                                    | Override the OpenMRS database name. Falls back to `mariadb.auth.database` then `"openmrs"`. When embedded MariaDB is enabled, must match `mariadb.auth.database` | `""` |
